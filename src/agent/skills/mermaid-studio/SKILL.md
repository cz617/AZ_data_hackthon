...
```bash
node .amandax/skills/mermaid-studio/scripts/validate.mjs <file.mmd>
```
...
```bash
bash .amandax/skills/mermaid-studio/scripts/setup.sh
```
...
node .amandax/skills/mermaid-studio/scripts/render.mjs -i diagram.mmd -o diagram.svg

# PNG (for documents/presentations)
node .amandax/skills/mermaid-studio/scripts/render.mjs -i diagram.mmd -o diagram.png --width 1200

# ASCII (for terminals/READMEs)
node .amandax/skills/mermaid-studio/scripts/render-ascii.mjs -i diagram.mmd

# With icon packs (architecture-beta with AWS/tech icons)
node .amandax/skills/mermaid-studio/scripts/render.mjs -i diagram.mmd -o diagram.svg --icons logos,fa
```
...
## Step 5: Generate from Database Schema

Generate Mermaid diagrams from distributed schema structure. Output is wrapped in markdown code blocks (`.md` format) for compatibility with GitHub, GitLab, and other markdown renderers.

**Schema Format:** See `references/schema-format-spec.md` for the distributed schema structure specification.
...
Generate Mermaid diagrams from distributed schema structure with simple regex search.

```bash
# First, export schema with db-toolkit
python .amandax/skills/db-toolkit/scripts/schema_profiler.py --database bird_testing --output-dir artifacts/schemas

# All tables
python .amandax/skills/mermaid-studio/scripts/generate_from_schema.py \
    --schema-dir artifacts/schemas/bird_testing \
    --output artifacts/diagrams/all.md

# Search by pattern (regex)
python .amandax/skills/mermaid-studio/scripts/generate_from_schema.py \
    --schema-dir artifacts/schemas/bird_testing \
    --pattern "user.*" \
    --output artifacts/diagrams/users.md

python .amandax/skills/mermaid-studio/scripts/generate_from_schema.py \
    --schema-dir artifacts/schemas/bird_testing \
    --pattern "account|card" \
    --output artifacts/diagrams/finance.md

# Lineage diagram (table level)
python .amandax/skills/mermaid-studio/scripts/generate_from_schema.py \
    --schema-dir artifacts/schemas/bird_testing \
    --type lineage \
    --direction LR \
    --output artifacts/diagrams/lineage.md

# Lineage diagram (column level - shows specific FK columns)
python .amandax/skills/mermaid-studio/scripts/generate_from_schema.py \
    --schema-dir artifacts/schemas/bird_testing \
    --type lineage \
    --lineage-level column \
    --direction LR \
    --output artifacts/diagrams/column_lineage.md

# With statistics
python .amandax/skills/mermaid-studio/scripts/generate_from_schema.py \
    --schema-dir artifacts/schemas/bird_testing \
    --pattern "account|district" \
    --stats \
    --output artifacts/diagrams/finance.md
```
...
|-----------|---------|-------------|
| `--pattern` | (all) | Regex for table names (e.g. `user.*`, `.*order.*`, `account\|card`) |
| `--type` | `er` | Diagram type: `er`, `lineage`, `schema` |
| `--direction` | `TB` | Flow direction: `TB`, `LR`, `RL`, `BT` |
| `--lineage-level` | `table` | Lineage granularity: `table` or `column` |
...
📊 Diagram Statistics:
  - Included tables: 4
  - Relationships: 4
  - Total tables in schema: 76
  - Isolated tables: cards
```

## Scripts

### generate_from_schema.py (Python)

Generate Mermaid diagrams from distributed schema. Output is Markdown format (.md).

```bash
# Generate ER relationship diagram
python scripts/generate_from_schema.py \
  --schema-dir artifacts/schemas/mydb \
  --type relationship \
  --output artifacts/diagrams/mydb.md
...
**Options:**
- `--type relationship|lineage` - Diagram type
- `--pattern "regex"` - Filter tables by regex pattern
- `--output file` - Output .md file (default: diagram.md)

### validate.mjs (Node.js)

Validate Mermaid syntax using official Mermaid library.

```bash
node scripts/validate.mjs artifacts/diagrams/diagram.md
```
...
```bash
node scripts/render.mjs artifacts/diagrams/diagram.md --output output.svg
```

### render-ascii.mjs (Node.js)

Render Mermaid to ASCII art for terminal output.

```bash
node scripts/render-ascii.mjs artifacts/diagrams/diagram.md
```
...
**统一输出目录**: `artifacts/diagrams/`

```
artifacts/diagrams/
├── *.md               # Mermaid 源文件 (markdown 格式，GitHub/GitLab 兼容)
├── *.svg              # SVG 渲染输出
├── *.png              # PNG 渲染输出
└── *.txt              # ASCII 渲染输出
```

- **Default output**: `.md` files with ` ```mermaid ... ``` ` code blocks (GitHub/GitLab compatible)
- **Legacy output**: `.mmd` files (use `--no-markdown` flag)
- Naming: `{purpose}-{type}.md` (e.g., `auth-flow-sequence.md`)
- Rendered outputs go to same directory with corresponding extension
- Always provide both the source file and rendered file to the user
